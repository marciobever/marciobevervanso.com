...
const CardsPage: React.FC<Props> = ({ onViewDetails }) => {
  ...
  useEffect(() => {
    document.title = "Cartões de Crédito Sem Anuidade e Para Negativado | Lista 2025";

    const metaDesc = document.querySelector('meta[name="description"]');
    const descContent = "Compare os melhores cartões de crédito de 2025. Aprovação facilitada para score baixo, sem consulta ao SPC/Serasa, opções sem anuidade e com limite alto.";
    if (metaDesc) {
    ...
    const keysContent = "cartão para negativado, cartão sem anuidade, cartão de crédito fácil aprovação, nubank, will bank, cartão com limite alto, score baixo";
    if (metaKeys) {
      metaKeys.setAttribute('content', keysContent);
    } else {
      const meta = document.createElement('meta');
      meta.name = "keywords";
      meta.content = keysContent;
      document.head.appendChild(meta);
    }
  }, []);
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="mt-20 max-w-4xl mx-auto prose prose-slate">
           ...
           <p className="text-slate-600">
             Conseguir crédito com o nome sujo pode parecer difícil, mas em 2025 diversas fintechs e bancos digitais especializaram-se neste público. 
